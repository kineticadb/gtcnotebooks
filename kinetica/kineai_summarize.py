...
class SqlAssistLLM(LoggingMixin):
    ...
    def _execute_sql(self, sql: str) -> list:
        response = self._dbc.execute_sql_and_decode(sql, limit=40, get_column_major=False)
...
class KineticaLLM(LoggingMixin):
    ...
    def __init__(self, sql_context: str = 'raceday.raceday_ui_ctxt'):
        self._nemo_summarize = NemoSummaryLLM()
        self._nemo_chat = NemoChatLLM()
        self._sqlAssist = SqlAssistLLM(sql_context)

    def chat(self, in_ctx: list, question: str) -> list:
        if question.startswith('Aerial'):
            nemo_ctx = self._nemo_summarize.summarize(in_ctx, question)
            return nemo_ctx

        sa_response = self._sqlAssist.query(question)
        if len(json.loads(sa_response)) < 4:
            try:
                nemo_ctx = self._nemo_summarize.summarize(in_ctx, f'{sa_response}') # {self.SA_PREFIX}
                return nemo_ctx
            except Exception as e:
                nemo_ctx = self._nemo_summarize.summarize(in_ctx, f'{sa_response}') # {self.SA_PREFIX}
                return nemo_ctx
        else:
            return HTML(pd.json_normalize(json.loads(sa_response)).to_html(index=False))

    def report(self, in_ctx: list, percentage_change: int) -> list:
        q1 = f'show me the top 5 stocks that have increased in value by {percentage_change}% or more in the last week with percentage increases'
        q2 = f'show me the top 5 stocks that have decreased in value by {percentage_change}% or more in the last week with percentage decreases'

        questions = [q1, q2]
        titles = ['TOP 5 Gains', 'TOP 5 Losses']
        data = ['can you please summarize the following JSON:']

        for idx, q in enumerate(questions):
            sa_response = self._sqlAssist.query(q)
            data.append(titles[idx])
            data.append(sa_response)

        nemo_ctx = self._nemo_summarize.summarize(in_ctx, '\n\n'.join(data))
